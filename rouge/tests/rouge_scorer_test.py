...
class RougeScorerTest(parameterized.TestCase):
    ...
    def testRougeLSumSentenceSplitting(self):
        ...
        scorer = rouge_scorer.RougeScorer(
            ["rougeLsum"], use_stemmer=True, split_summaries=False
        )
        result = scorer.score(target, prediction)
        ...
        scorer = rouge_scorer.RougeScorer(
            ["rougeLsum"], use_stemmer=True, split_summaries=True
        )
        result = scorer.score(target, prediction)
    ...
    def testLcsTable(self):
        ...
        # Indices
        self.assertEqual([1, 2, 3], _read_lcs(t, ref, c1))
        # Values
        self.assertEqual([2, 3, 4], [ref[i] for i in _read_lcs(t, ref, c1)])

        # No common subsequence.
        c2 = [8, 9]
        t = rouge_scorer._lcs_table(ref, c2)
        self.assertEqual(0, t[len(ref)][len(c2)])
        self.assertEqual([], _read_lcs(t, ref, c2))
    ...
    def testUnionLcs(self):
        ...
        self.assertEqual([1, 2, 3, 5], rouge_scorer._union_lcs(ref, [c1, c2]))
        self.assertEqual([1, 2, 3, 5], rouge_scorer._union_lcs(ref, [c1, c2]))

    def testSummaryLevelLcs(self):
        refs = [[1, 2, 3, 4, 5]]
        cans = [[1, 2, 6, 7, 8], [1, 3, 8, 9, 5]]  # lcs = [1, 2]  # lcs = [1, 3, 5]
    ...
    def testRougeTokenizerInit(self):
        scorer = rouge_scorer.RougeScorer(
            ["rouge1"], tokenizer=tokenizers.DefaultTokenizer()
        )
