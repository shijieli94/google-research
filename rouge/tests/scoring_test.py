...
class BootstrapAggregatorTest(absltest.TestCase):
    ...
    def assertSimilarAggregates(
        self, precision, recall, fmeasure, aggregate, delta=_DELTA
    ):
    ...
    def testConfidenceIntervalsAgainstRouge155WithStemming(self):
        ...
        self.assertSimilarAggregates(
            (0.51027, 0.52434, 0.53788),
            (0.32563, 0.33580, 0.34548),
            (0.39380, 0.40524, 0.41661),
            result["rouge1"],
        )
        self.assertSimilarAggregates(
            (0.50759, 0.52104, 0.53382),  # P
    ...
    def testConfidenceIntervalsAgainstRouge155WithStemmingMultiLine(self):
        ...
        aggregator = scoring.BootstrapAggregator()
        t_files = [
            os.path.join(test_util.PYROUGE_DIR, "target_multi.%d.txt" % i)
            for i in range(0, 250)
        ]
        p_files = [
            os.path.join(test_util.PYROUGE_DIR, "prediction_multi.%d.txt" % i)
            for i in range(0, 250)
        ]
